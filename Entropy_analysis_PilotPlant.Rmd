...
concentrations <- read_delim("mass_flows_and_concentrations.CSV", 
    delim = ";", escape_double = FALSE, locale = locale(decimal_mark = ",", 
        grouping_mark = "."), trim_ws = TRUE) 
nms_concentrations <- concentrations$Stream
concentrations <- concentrations %>% select(-c(Stream,`Mass
flowrate`)) %>% as.data.frame()
row.names(concentrations) <-  nms_concentrations
...
of.interest <- c("Cassiterite","Iron Oxide","Cu Sulphides","Arsenopyrite","Sphalerite")
...
batch_entropy.parallel <- function(Sample,part.numbers){
  pd <- MLA.sample[[1]]
  idx <- sample(1:nrow(pd),size = part.numbers,replace = T)

  pd <- pd[idx,]
  nm <- names(MLA.sample)
  entropy = batch.entropy.norm(pd,sample_point = nm)
  return(entropy)
}

batch_entropy.parallel(part_dat["S01"],part.numbers = 10000)
```
...
Entropies_ungrouped <-  map(sample_points, ~future_replicate(5,batch_entropy.parallel(sample_point = .x,part.numbers = 10000,particle_data = part_dat),future.seed = T,simplify = F))
Entropies.agg_ungrouped <- Entropies_ungrouped %>% map(~bind_rows(.x,.id = "N")) %>% bind_rows(.id = "Sample")
...
Entropies_grouped <-  map(sample_points, ~future_replicate(20,batch_entropy.parallel(sample_point = .x,part.numbers = 100000,particle_data = part_dat_grouped),future.seed = T,simplify = F))
...
Entropies.agg_grouped <- Entropies.agg_grouped %>%
  group_by(Sample,entropy_type,aggregation) %>%
  summarise(mean_entropy = mean(entropy), sd_entropy = sd(entropy))
save(Entropies.agg_grouped,file = "Entropies_grouped_conccorr.RData")
...
#plot for sieve
sieve_data_product <- Entropies.agg_grouped_1g %>% filter(Sample %in% machines$Sieve$product_stage)
sieve_data_feed <- Entropies.agg_grouped_1g %>% filter(Sample %in% machines$Sieve$feed_stage)
...
spiral_data_product <- Entropies.agg_grouped_1g %>% filter(Sample %in% machines$`Spiral Con.`$product_stage)
spiral_data_feed <- Entropies.agg_grouped_1g %>% filter(Sample %in% machines$`Spiral Con.`$feed_stage)
...
#plot for mill1
#plot for spiral classifier 
mill1_data_product <- Entropies.agg_grouped_1g %>% filter(Sample %in% machines$Mill1$product_stage)
mill1_data_feed <- Entropies.agg_grouped_1g %>% filter(Sample %in% machines$Mill1$feed_stage)
...
mag_sep_data_product <- Entropies.agg_grouped_1g %>% filter(Sample %in% machines$`Magnetic Con.`$product_stage)
mag_sep_data_feed <- Entropies.agg_grouped_1g %>% filter(Sample %in% machines$`Magnetic Con.`$feed_stage)
...
flotation_data_product <- Entropies.agg_grouped_1g %>% filter(Sample %in% machines$`Sulfide Flotation`$product_stage)
flotation_data_feed <- Entropies.agg_grouped_1g %>% filter(Sample %in% machines$`Sulfide Flotation`$feed_stage)
...
screw_class_data_product <- Entropies.agg_grouped_1g %>% filter(Sample %in% machines$`Screw Class.`$product_stage)
screw_class_data_feed <- Entropies.agg_grouped_1g %>% filter(Sample %in% machines$`Screw Class.`$feed_stage)
...
hydrocyclone_data_product <- Entropies.agg_grouped_1g %>% filter(Sample %in% machines$Hydrocyclone$product_stage)
hydrocyclone_data_feed <- Entropies.agg_grouped_1g %>% filter(Sample %in% machines$Hydrocyclone$feed_stage)
...
shaking_table_data_product <- Entropies.agg_grouped_1g %>% filter(Sample %in% machines$`Shaking Table`$product_stage)
shaking_table_data_feed <- Entropies.agg_grouped_1g %>% filter(Sample %in% machines$`Shaking Table`$feed_stage)
...
mill2_data_product <- Entropies.agg_grouped_1g %>% filter(Sample %in% machines$Mill2$product_stage)
mill2_data_feed <- Entropies.agg_grouped_1g %>% filter(Sample %in% machines$Mill2$feed_stage)
...
ggsave(filename = "batch.entropies_conccorr.png",device = "png",width = 20, height = 50, units = "cm")
```
...
entropies_agg <- entropies %>% bind_rows(.id = "N") %>% group_by(stage,aggregation,entropy_type) %>% 
  summarise(mean_entropy = mean(entropy),sd_entropy = sd(entropy))
return(entropies_agg)
}
#test_1 <- machine.entropy(machines$Hydrocyclone,particle.numbers = particle.numbers,total.number = 10000,replicates = 20)
#test_2 <- machine.entropy(machines$Mill2,particle.numbers = particle.numbers, total.number = 10000, replicates = 20)
