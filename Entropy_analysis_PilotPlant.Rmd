...
# save(Entropies.agg_grouped_corrected,file = "Entropies_grouped_corrected.RData")
 ...
 load("Entropies_grouped_uncorrected.RData")
 load("Entropies_grouped_corrected.RData")
...
```{r}
Part.Entropies <- Entropies.agg_grouped_uncorrected %>% filter(entropy_type %in% c("p00k","p0jk")) %>% 
  right_join(num.res,by = "Sample") %>% 
  mutate(mean_entropy = mean_entropy + log(particle.numbers)) %>% 
  select(-particle.numbers)

Chem.Entropies <- Entropies.agg_grouped_uncorrected %>% filter(!(entropy_type %in% c("p00k","p0jk")))

Entropies.agg_grouped_uncorrected_1g <- bind_rows(Part.Entropies,Chem.Entropies) %>% arrange(Sample) %>% 
  rename( type = entropy_type)

g <- ggplot(Entropies.agg_grouped_uncorrected_1g,aes(x = Sample,y = mean_entropy, fill = type)) +
  geom_bar(stat = "identity") +
...
ggsave(filename = "entropy.batches.normalized_grouped_uncorrected.png",device = "png",width = 30, height = 20, units = "cm")

Part.Entropies <- Entropies.agg_grouped_corrected %>% filter(entropy_type %in% c("p00k","p0jk")) %>% 
  right_join(num.res,by = "Sample") %>% 
  mutate(mean_entropy = mean_entropy + log(particle.numbers)) %>% 
  select(-particle.numbers)

Chem.Entropies <- Entropies.agg_grouped_corrected %>% filter(!(entropy_type %in% c("p00k","p0jk")))

Entropies.agg_grouped_corrected_1g <- bind_rows(Part.Entropies,Chem.Entropies) %>% arrange(Sample) %>% 
  rename( type = entropy_type)

g <- ggplot(Entropies.agg_grouped_corrected_1g,aes(x = Sample,y = mean_entropy, fill = type)) +
  geom_bar(stat = "identity") +
  facet_grid(.~aggregation) +
  theme_classic() +
  labs(x = "Sample Point", y = "entropy_value") +
...
#plot for sieve
sieve_data_product <- Entropies.agg_grouped_corrected_1g %>% filter(Sample %in% machines$Sieve$product_stage)
sieve_data_feed <- Entropies.agg_grouped_corrected_1g %>% filter(Sample %in% machines$Sieve$feed_stage)
...
sieve <- ggplot(sieve_data,aes(x = Sample, y = mean_entropy, fill = type )) +
  ...
  scale_fill_manual(values = entropy_colors) +
  labs(x = "Sample Point", y = "entropy value", title = "Sieve") +
  theme_classic()
plot(sieve)

#plot for spiral classifier 
spiral_data_product <- Entropies.agg_grouped_corrected_1g %>% filter(Sample %in% machines$`Spiral Con.`$product_stage)
spiral_data_feed <- Entropies.agg_grouped_corrected_1g %>% filter(Sample %in% machines$`Spiral Con.`$feed_stage)
...
spiral <- ggplot(spiral_data,aes(x = Sample, y = mean_entropy, fill = type )) +
  ...
  facet_grid(type~stage,labeller = labeller(stage = c("1" = "Feed Stage","2" = "Product Stage")), scales = "free") +
  scale_fill_manual(values = entropy_colors) +
  labs(x = "Sample Point", y = "entropy value", title = "Spiral Classifier") +
  theme_classic()
plot(spiral)

#plot for mill1
#plot for spiral classifier 
mill1_data_product <- Entropies.agg_grouped_corrected_1g %>% filter(Sample %in% machines$Mill1$product_stage)
mill1_data_feed <- Entropies.agg_grouped_corrected_1g %>% filter(Sample %in% machines$Mill1$feed_stage)
...
mill1 <- ggplot(mill1_data,aes(x = Sample, y = mean_entropy, fill = type )) +
  ...
  geom_errorbar(aes(ymin=mean_entropy-sd_entropy, ymax=mean_entropy+sd_entropy), width=.2,
   ...
   facet_grid(type~stage,labeller = labeller(stage = c("1" = "Feed Stage","2" = "Product Stage")), scales = "free") +
  scale_fill_manual(values = entropy_colors) +
  labs(x = "Sample Point", y = "entropy value", title = "Mill1") +
  theme_classic()
plot(mill1)

#plot for magnetic separation

mag_sep_data_product <- Entropies.agg_grouped_corrected_1g %>% filter(Sample %in% machines$`Magnetic Con.`$product_stage)
mag_sep_data_feed <- Entropies.agg_grouped_corrected_1g %>% filter(Sample %in% machines$`Magnetic Con.`$feed_stage)
...
mag_sep <- ggplot(mag_sep_data,aes(x = Sample, y = mean_entropy, fill = type )) +
  ...
  facet_grid(type~stage,labeller = labeller(stage = c("1" = "Feed Stage","2" = "Product Stage")), scales = "free") +
  scale_fill_manual(values = entropy_colors) +
  labs(x = "Sample Point", y = "entropy value", title = "Magnetic Separation") +
  theme_classic()
plot(mag_sep)

#plot for sulfide flotation

flotation_data_product <- Entropies.agg_grouped_corrected_1g %>% filter(Sample %in% machines$`Sulfide Flotation`$product_stage)
flotation_data_feed <- Entropies.agg_grouped_corrected_1g %>% filter(Sample %in% machines$`Sulfide Flotation`$feed_stage)
...
flotation <- ggplot(flotation_data,aes(x = Sample, y = mean_entropy, fill = type )) +
  ...
  facet_grid(type~stage,labeller = labeller(stage = c("1" = "Feed Stage","2" = "Product Stage")), scales = "free") +
  scale_fill_manual(values = entropy_colors) +
  labs(x = "Sample Point", y = "entropy value", title = "Sulfide Flotation") +
  theme_classic()
plot(flotation)

screw_class_data_product <- Entropies.agg_grouped_corrected_1g %>% filter(Sample %in% machines$`Screw Class.`$product_stage)
screw_class_data_feed <- Entropies.agg_grouped_corrected_1g %>% filter(Sample %in% machines$`Screw Class.`$feed_stage)
...
screw_class <- ggplot(screw_class_data,aes(x = Sample, y = mean_entropy, fill = type )) +
  ...
  facet_grid(type~stage,labeller = labeller(stage = c("1" = "Feed Stage","2" = "Product Stage")), scales = "free") +
  scale_fill_manual(values = entropy_colors) +
  labs(x = "Sample Point", y = "entropy value", title = "Screw Classifier") +
  theme_classic()
plot(screw_class)

hydrocyclone_data_product <- Entropies.agg_grouped_corrected_1g %>% filter(Sample %in% machines$Hydrocyclone$product_stage)
hydrocyclone_data_feed <- Entropies.agg_grouped_corrected_1g %>% filter(Sample %in% machines$Hydrocyclone$feed_stage)
...
hydrocyclone <- ggplot(hydrocyclone_data,aes(x = Sample, y = mean_entropy, fill = type )) +
  ...
  facet_grid(type~stage,labeller = labeller(stage = c("1" = "Feed Stage","2" = "Product Stage")), scales = "free") +
  scale_fill_manual(values = entropy_colors) +
  labs(x = "Sample Point", y = "entropy value", title = "Hydrocyclone") +
  theme_classic()
plot(hydrocyclone)

shaking_table_data_product <- Entropies.agg_grouped_corrected_1g %>% filter(Sample %in% machines$`Shaking Table`$product_stage)
shaking_table_data_feed <- Entropies.agg_grouped_corrected_1g %>% filter(Sample %in% machines$`Shaking Table`$feed_stage)
...
shaking_table <- ggplot(shaking_table_data,aes(x = Sample, y = mean_entropy, fill = type )) +
  ...
  facet_grid(type~stage,labeller = labeller(stage = c("1" = "Feed Stage","2" = "Product Stage")), scales = "free") +
  scale_fill_manual(values = entropy_colors) +
  labs(x = "Sample Point", y = "entropy value", title = "Shaking Table") +
  theme_classic()
plot(shaking_table)

#mill2 

mill2_data_product <- Entropies.agg_grouped_corrected_1g %>% filter(Sample %in% machines$Mill2$product_stage)
mill2_data_feed <- Entropies.agg_grouped_corrected_1g %>% filter(Sample %in% machines$Mill2$feed_stage)
...
mill2 <- ggplot(mill2_data,aes(x = Sample, y = mean_entropy, fill = type )) +
  ...
  facet_grid(type~stage,labeller = labeller(stage = c("1" = "Feed Stage","2" = "Product Stage")), scales = "free") +
  scale_fill_manual(values = entropy_colors) +
  labs(x = "Sample Point", y = "entropy value", title = "Mill2") +
  theme_classic()
plot(mill2)
...
ggsave(filename = "batch.entropies_grouped_corrected.png",device = "png",width = 20, height = 50, units = "cm")
```
...
if(pl > 1){
entropy <- entropyforboots.corrected(boots_f_recal,boots_p,weights = machine$weights,recal = T, con.corr = F,idx = idx)
}else{
entropy <- entropyforboots.corrected(boots_f,boots_p,weights = machine$weights, recal = F, con.corr = F,idx = idx)
}
...
availableCores()
plan(multisession,workers = availableCores() -1)
#entropies <- parallel.calculation(samples = samples,numbers = p_num)
entropies <-  future_replicate(replicates,parallel.calculation(samples = samples,numbers = p_num),future.seed = T,simplify = F)
...
machine_entropies <- map(machines,~machine.entropy(.x,particle.numbers = particle.numbers,total.number = 100000,replicates = 250)) 
save(machine_entropies,file = "machine_entropies.RData")
load("./machine_entropies.RData")


machine_entropies_uncorr <- map(machines,~machine.entropy(.x,particle.numbers = particle.numbers,total.number = 100000,replicates = 250)) 
save(machine_entropies_uncorr,file = "machine_entropies_uncorr.RData")
load("./machine_entropies_uncorr.RData")
machine_entropies_uncorr <- map(machine_entropies_uncorr,transpose)
t_values_uncorr <- map(machine_entropies_uncorr,~.x$t_values)
machine_entropies_agg <- map(machine_entropies_uncorr,~.x$entropy)
...
ggsave(filename = "machine.entropies.corr.png",device = "png",width = 30, height = 20, units = "cm")
